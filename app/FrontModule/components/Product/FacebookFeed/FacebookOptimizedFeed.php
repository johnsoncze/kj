 ...
 */
class FacebookOptimizedFeed extends AbstractFeed
{
    /** @var string */
    const CACHE_TAG = 'facebookOptimizedFeed';
    ...
    public function __construct(NetteDatabase $ndb, LockFacadeFactory $lockFacadeFactory)
    {
        ...
        $this->templatePath = __DIR__ . '/facebookFeed.latte';
    }
    ...
    private function setProductsDataToTemplate(): void
    {
	    ...
	    $this->template->availabulityDate = (new DateTime())->modify('+1 month')->format(DateTime::RFC3339);
    ...
    public function getProductsData(int $limit = 0): array
    {
        return $this->ndb->query("
            SELECT product.p_id, product.p_code, pt_google_merchant_title, pt_name, p_google_merchant_category, p_google_merchant_brand_text, 
                   product_translation.pt_description, product_translation.pt_url, product.p_stock, p_empty_stock_state, product.p_photo, product.p_price
            FROM product
            ...
            AND p_id NOT IN (SELECT pv_product_variant_id FROM product_variant)
            LIMIT ?
        ",
            Product::PUBLISH,
            $limit ?: PHP_INT_MAX
